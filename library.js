...
if (storage.length > 0 && storage.getItem("library") != null) {
...
function addBookToLibrary(title, author, numPages, read) {
  const book = new Book(
    title,
    author,
    Intl.NumberFormat().format(numPages),
    read
  );
...
function refreshHTMLTable() {
  ...
  myLibrary.forEach((book) => {
    ...
    for (const property in book) {
      //loop through book properties adding cells to the row
...
function updateCheckBox(e) {
  myLibrary[e.target.parentElement.parentElement.rowIndex - 1].hasBeenRead =
    e.target.checked;
  if (storage !== "undefined")
    storage.setItem("library", JSON.stringify(myLibrary));
}
